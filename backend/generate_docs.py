"""
Complete Documentation Generator for Tamargi.ai (Audited & Factually Verified Version)
Generates:
  1. docs/Tamargi_AI_Project_Documentation.md
  2. docs/Tamargi_AI_Project_Documentation.docx
"""
...
MD_PATH = DOCS_DIR / "Tamargi_AI_Project_Documentation.md"
DOCX_PATH = DOCS_DIR / "Tamargi_AI_Project_Documentation.docx"

def get_full_markdown_text():
    return """# Tamargi.ai
...
$$RRF(d) = \frac{w_{dense}}{k + rank_{dense}(d)} + \frac{w_{BM25}}{k + rank_{BM25}(d)}$$
...
def generate_docx_documentation(md_text: str, docx_path: Path):
    doc = docx.Document()
    ...
    for section in doc.sections:
        ...
        f_run = f_p.add_run("Tamargi.ai — Technical Project Documentation | Page ")
    ...
    t_run = title_p.add_run("Tamargi.ai")
    ...
    s_run = sub_p.add_run("Explainable Personalized Medication Safety RAG System for Egyptian Patients")
    s_run.font.name = "Calibri"
    s_run.font.size = Pt(16)
    s_run.font.italic = True
    s_run.font.color.rgb = RGBColor(70, 80, 95)

    desc_p = doc.add_paragraph()
    desc_p.paragraph_format.space_after = Pt(120)
    d_run = desc_p.add_run("Complete Engineering, Research, Testing, and Architectural Documentation")
    d_run.font.name = "Calibri"
    d_run.font.size = Pt(12)
    d_run.font.color.rgb = RGBColor(100, 110, 120)

    meta_p = doc.add_paragraph()
    meta_p.paragraph_format.space_after = Pt(6)
    m1 = meta_p.add_run("Author: ")
    m1.bold = True
    meta_p.add_run("Tamargi.ai Engineering & Research Team\n")
    m2 = meta_p.add_run("Project Repository: ")
    m2.bold = True
    meta_p.add_run("MohameddTamerr/tamargi-pharma-rag\n")
    m3 = meta_p.add_run("Regulatory Authority: ")
    m3.bold = True
    meta_p.add_run("Egyptian Drug Authority (EDA) Guidelines & Formularies\n")
    m4 = meta_p.add_run("Document Version: ")
    m4.bold = True
    meta_p.add_run("2.0 (Audited & Production Verified)\n")
    m5 = meta_p.add_run("Date: ")
    m5.bold = True
    ...
    def flush_table(t_lines):
        if not t_lines:
            return
        rows = [r.strip().strip("|").split("|") for r in t_lines if not re.match(r"^\s*\|?[\s\-:|]+\|?\s*$", r)]
        if not rows:
            return
        ...
        for row_idx, row_data in enumerate(rows):
            for col_idx in range(num_cols):
                cell_text = clean_xml_string(row_data[col_idx].strip()) if col_idx < len(row_data) else ""
                cell = tbl.cell(row_idx, col_idx)
                cell.text = cell_text
                tcPr = cell._tc.get_or_add_tcPr()
                if row_idx == 0:
                    shd = parse_xml(f'<w:shd {nsdecls("w")} w:fill="0F2D5A"/>')
                    tcPr.append(shd)
    ...
    for line in lines:
        ...
        if line.startswith("# "):
            ...
            r = p.add_run(clean_xml_string(line[2:].strip()))
            r.font.name = "Arial"
            r.font.size = Pt(20)
            r.font.bold = True
            r.font.color.rgb = RGBColor(15, 45, 90)
        elif line.startswith("## "):
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(14)
            p.paragraph_format.space_after = Pt(6)
            r = p.add_run(clean_xml_string(line[3:].strip()))
            r.font.name = "Arial"
            r.font.size = Pt(15)
            r.font.bold = True
            r.font.color.rgb = RGBColor(25, 65, 120)
        elif line.startswith("### "):
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(10)
            p.paragraph_format.space_after = Pt(4)
            r = p.add_run(clean_xml_string(line[4:].strip()))
            r.font.name = "Arial"
            r.font.size = Pt(12.5)
            r.font.bold = True
            r.font.color.rgb = RGBColor(40, 75, 130)
        elif line.startswith("#### "):
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(8)
            p.paragraph_format.space_after = Pt(3)
            r = p.add_run(clean_xml_string(line[5:].strip()))
            r.font.name = "Calibri"
            r.font.size = Pt(11)
            r.font.bold = True
            r.font.color.rgb = RGBColor(50, 60, 70)
        elif line.startswith("- ") or line.startswith("* "):
            p = doc.add_paragraph(style='List Bullet')
            p.paragraph_format.space_after = Pt(3)
            p.paragraph_format.line_spacing = 1.15
            parse_inline_formatting(p, line[2:].strip())
            ...
            p.paragraph_format.line_spacing = 1.15
            if prefix_match:
                parse_inline_formatting(p, prefix_match.group(2).strip())
            else:
...
def main():
    md_content = get_full_markdown_text()
    with open(MD_PATH, "w", encoding="utf-8") as f:
        f.write(md_content)
    print(f"Generated Markdown: {MD_PATH}")

    generate_docx_documentation(md_content, DOCX_PATH)
